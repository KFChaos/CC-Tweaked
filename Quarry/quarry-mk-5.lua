    ...
    for i = 1, 16 do
        ...
        if item then
            if item.name == "minecraft:cobblestone" 
                or item.name == "minecraft:stone" 
                or item.name == "minecraft:gravel" 
                or item.name == "minecraft:flint" 
                or item.name == "minecraft:wheat_seeds" 
                or item.name == "minecraft:granite" 
                or item.name == "minecraft:diorite" 
                or item.name == "minecraft:andesite" 
                or item.name == "minecraft:deepslate" 
                or item.name == "minecraft:cobbled_deepslate" 
                or item.name == "minecraft:calcite" 
                or item.name == "minecraft:tuff"
                or item.name == "minecraft:smooth_basalt" 
                or item.name == "minecraft:dirt"
                or item.name == "minecraft:netherrack" 
                or item.name == "minecraft:soul_sand" 
                or item.name == "minecraft:soul_soil" 
                or item.name == "minecraft:basalt" 
                or item.name == "minecraft:polished_basalt" 
                or item.name == "minecraft:blackstone" 
                or item.name == "minecraft:nether_wart_block" 
                or item.name == "quark:cobbled_deepslate"
                or item.name == "quark:smooth_basalt"
                or item.name == "create:limestone_cobblestone"
                or item.name == "create:weathered_limestone_cobblestone"
                or item.name == "create:gabbro_cobblestone"
                or item.name == "create:dolomite_cobblestone"
                or item.name == "create:scoria" then
                turtle.select(i)
                turtle.dropDown()
...
function main()
    ...
    -- print("I am at (" .. xCoord .. ", " .. yCoord .. ", " .. zCoord .. ")")
    orientation = tonumber(quarryCall[1])
    local width = tonumber(quarryCall[2])
    local depth = tonumber(quarryCall[3])
    if tableSize == 5 and quarryCall[4] == "F" and quarryCall[5] == "F" then
        homeX = xCoord
        homeY = yCoord
        homeZ = zCoord
        homeOrientation = orientation
        quarry(width, depth)
    elseif tableSize == 9 and quarryCall[4] == "T" and quarryCall[9] == "F" then
        homeX = tonumber(quarryCall[5])
        homeY = tonumber(quarryCall[6])
        homeZ = tonumber(quarryCall[7])
        homeOrientation = tonumber(quarryCall[8])
        quarry(width, depth)
    elseif tableSize == 9 and quarryCall[4] == "F" and quarryCall[5] == "T" then
        homeX = xCoord
        homeY = yCoord
        homeZ = zCoord
        homeOrientation = orientation
        xDest = tonumber(quarryCall[6])
        yDest = tonumber(quarryCall[7])
        zDest = tonumber(quarryCall[8])
        dirDest = tonumber(quarryCall[9])
        pathToAvoid(xDest, yDest, zDest, dirDest)
        quarry(width, depth)
    elseif tableSize == 13 and quarryCall[4] == "T" and quarryCall[9] == "T" then
        homeX = tonumber(quarryCall[5])
        homeY = tonumber(quarryCall[6])
        homeZ = tonumber(quarryCall[7])
        homeOrientation = tonumber(quarryCall[8])
        xDest = tonumber(quarryCall[10])
        yDest = tonumber(quarryCall[11])
        zDest = tonumber(quarryCall[12])
        dirDest = tonumber(quarryCall[13])
        pathToAvoid(xDest, yDest, zDest, dirDest)
        quarry(width, depth)
    else
